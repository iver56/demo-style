...
def run(content_image_dir, style_image_dir, host_name, overwrite):
    ...
    for content_filename in tqdm(content_filenames):
        for style_filename in style_filenames:
            ...
            output_image_file_path = os.path.join(
                STYLIZED_IMAGE_DIR, content_filename + "__" + style_filename + ".png"
            )
            if os.path.isfile(output_image_file_path) and not overwrite:
                print(
                    "Skipping, because {} already exists".format(output_image_file_path)
                )
                continue

            content_image = Image.open(
                os.path.join(content_image_dir, content_filename)
            )
            ...
            try:
                content_segmentation_image = Image.open(
                    os.path.join(content_image_dir, content_label_filename)
                )
                style_segmentation_image = Image.open(
                    os.path.join(style_image_dir, style_label_filename)
                )
            ...
            data = {
                ...
                "content_segmentation_base64": pil_image_to_base64(
                    content_segmentation_image
                ),
                "style_image_base64": pil_image_to_base64(style_image),
                "style_segmentation_base64": pil_image_to_base64(
                    style_segmentation_image
                ),
            }

            r = requests.post(rest_api_url, json=data, timeout=60)
            json_data = r.json()
            output_image = base64_png_image_to_pillow_image(
                json_data["output_image_base64"]
            )
            output_image.save(output_image_file_path, format="PNG")
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--overwrite",
        dest="overwrite",
        help="By default, existing files are not overwritten. This option enables"
        " overwriting.",
        nargs="?",
        const=True,
        required=False,
